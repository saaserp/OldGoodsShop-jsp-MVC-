(function(jQuery){
	...
	var $version='2.5.2';
	var $athor='AJan';
	var $email='971546@qq.com';
	var $date='2010-12-28 19:50';
	var $o;
	var $opt;
	var $x, $y;
	var $bWidth, $bHeight;
	...
	this.init=function(options){
		$opt = $.extend({title: $name+' '+$version, width: 380, height: 220, left: '50%', top: '50%', html: '', url: '', mask: true, drag: false, btnClose: true, callback: function(){}}, options);
		$opt.oleft = $opt.left;
		$opt.otop = $opt.top;
		...
		$($tmps).show();
		$('#xPopupContent').css({height: $opt.height - 30 +'px'});
		if($isIE6){
		...
		if($eventBindCounter==0){
			...
			$('#xPopupBtnClose').click(function(){
				if((typeof $opt.callback=='function') && $opt.callback()==false){return false;}
				xHide();
			});
			$eventBindCounter=1;
		};
